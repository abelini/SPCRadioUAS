...
class ReportesCabinasController extends AppController
{
	...
	protected function getReportByProgram(DateTime $start, DateTime $end): Response
	{
		...
		$programa = $this->ProgramsRepository->loadInto($programa, [
			'ReportesProgramas' => function (SelectQuery $query) use ($start, $end) {
			...
			->formatResults(function (CollectionInterface $results) {
				return $results->map(function ($programa) {
					...
					if (!isset($programa['reportes']['V']))
						$programa['reportes']['V'] = [];
					if (!isset($programa['reportes']['G']))
						$programa['reportes']['G'] = [];
					if (!isset($programa['reportes']['S']))
						$programa['reportes']['S'] = [];
					if (!isset($programa['reportes']['X']))
						$programa['reportes']['X'] = [];
					$programa['reportes'] = [
	...
	protected function setViewVars(): void
	{
		...
		krsort($cuatrimestres, SORT_NUMERIC);
		foreach ($cuatrimestres as $i => $c) {
			krsort($cuatrimestres[$i], SORT_NATURAL);
		}
