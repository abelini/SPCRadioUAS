...
class CpanelDnsService
{
    ...
    public function __construct()
    {
        ...
        $this->scheme = Configure::read('SSLGeneration.cpanel.scheme');
        $this->host = Configure::read('SSLGeneration.cpanel.host');
        $this->port = Configure::read('SSLGeneration.cpanel.port');
    ...
    private function validateResponse(Response $response, string $context = 'cPanel API'): array
    {
        if ($response->getStatusCode() >= 400) {
            throw new RuntimeException($context . ' respondió con HTTP ' . $response->getStatusCode());
        }
        ...
        if (($payload['status'] ?? 0) !== 1) {
            ...
            throw new RuntimeException($context . ' falló: ' . $errors);
        }
    ...
    public function addTXTRecord(string $domain, string $value): void
    {
        ...
        $this->validateResponse($response, 'cPanel mass_edit_zone (add)');
    }
    ...
    public function removeTXTRecord(string $domain, string $value): void
    {
        ...
        foreach ($zoneData['records'] as $record) {
            ...
            $this->validateResponse($response, 'cPanel mass_edit_zone (remove)');
    ...
    private function fetchZoneData(): array
    {
        ...
        $payload = $this->validateResponse($response, 'cPanel parse_zone');
