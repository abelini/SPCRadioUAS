<div class="page-header">
    <h5><i class="fa-solid fa-folder-open"></i> Modificar solicitud #<?= $solicitud->ID ?></h5>
</div>

<div class="form-container">
    <?= $this->Form->create($solicitud, ['type' => 'PUT']) ?>

    <div class="form-group">
        <?= $this->Form->label('tipoSolicitudID', 'Tipo de solicitud') ?>
        <?= $this->Form->select('tipoSolicitudID', $tipos, ['class' => 'form-control']) ?>
    </div>
    ...
    <div class="form-group">
        ...
        <?= $this->Form->select('productorID', $productorTecnico, ['class' => 'form-control']) ?>
    </div>
...
<script type="text/javascript">
    ...
    jQuery('#fecha').datetimepicker({format:'Y-m-d H:i', lang:'es', minDate:0, step:30,
        value:'<?= $solicitud->fecha->format('Y-m-d H:i') ?>',
        i18n:{
            es:{
