...
<div class="row g-3">
	<div class="col-lg-3">
		<p style="color: var(--color-faded-silver);"><i class="fa-solid fa-list-check"></i> Hay
			<?= $reportes->count() ?> registros en las bitácoras del período.</p>

		<p style="color: var(--color-faded-silver);"><i class="fa-regular fa-circle-check"></i> Este programa tiene un
			cumplimiento del:
			<strong
				style="color: var(--color-ghost-white);"><?= $this->Number->toPercentage((count($ocurrences['V']) + count($ocurrences['G']) + count($ocurrences['S'])) / $reportes->count(), 1, ['multiply' => true]) ?></strong>.
		</p>

		<p style="color: var(--color-faded-silver);"><i class="fa-regular fa-circle-xmark"></i> <strong
				style="color: var(--color-ghost-white);"><?= count($ocurrences['X']) ?>
