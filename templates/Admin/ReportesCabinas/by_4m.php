	...
	<div class="row g-3">
		<div class="col-lg-3">
			<div class="alert alert-info">
				<p><i class="fa-solid fa-list-check"></i> Hay <strong><?= $reportesProgramas->count() ?>
						registros</strong> distribuídos en <strong><?= $programsCount ?> programas</strong> en las
					bitácoras del mes.</p>
			</div>

			<div class="alert alert-warning">
				<p><i class="fa-solid fa-satellite-dish"></i> Se enlazó por control remoto
					<strong><?= $reportesCR->sumOf('controles') ?> ocasiones</strong> en los
					<?= $reportStart->diffInDays($reportEnd) + 1 ?> días que abarca el presente cuatrimestre. Esto
					representa una media de
					<strong><?= round(($reportStart->diffInDays($reportEnd) + 1) / $reportesCR->sumOf('controles'), 2) ?></strong>
					enlaces por día.
				</p>
			</div>

			<div class="alert alert-success">
				<p><i class="fa-regular fa-circle-check"></i> El cumplimiento general en el mes es de:
					<strong><?= $this->Number->toPercentage((count($RPByStatus['V']) + count($RPByStatus['G']) + count($RPByStatus['S'])) / $reportesProgramas->count(), 1, ['multiply' => true]) ?></strong>.
				</p>
			</div>

			<div class="alert alert-danger">
				<p><i class="fa-regular fa-circle-xmark"></i> <strong><?= count($RPByStatus['X']) ?>
						(<?= $XtoWord ?>)</strong> faltas registradas.</p>
			</div>
		</div>
	...
	<ul class="cr-list">
		<?php foreach ($crs as $cr): ?>
			<li title="<?= $this->Time->i18nFormat(\DateTime::createFromFormat('U', $cr['fecha']), "d 'de' MMMM yyyy") ?>">
				...
				<span class="cr-date"><?= $this->Time->i18nFormat(\DateTime::createFromFormat('U', $cr['fecha']), 'd MMM') ?></span>
			</li>
		<?php endforeach; ?>
	</ul>
	...
	<div class="row g-3">
		<?php foreach ($programas as $programa): ?>
			<div class="col-12 col-md-6 col-lg-3"
				style="border-bottom: 1px solid var(--color-subtle-gray); padding: var(--spacing-8);">
				<div id="chartForP<?= $programa['ID'] ?>"
					style="padding: var(--spacing-8); width: 256px; height: 128px; margin: 0 auto;"></div>
				<p style="text-align: center; clear: both; margin: var(--spacing-4) 0;">
					<?= $programa['name'] ?><br />
					<span
						style="font-weight: bold;"><?= $this->Number->toPercentage($programa['chart']['Cumplimiento'], 1, ['multiply' => true]) ?></span>
				</p>
			</div>
		<?php endforeach; ?>
	</div>
