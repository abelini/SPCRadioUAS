			...
			<?php foreach ($asignaciones as $dateKey => $asignaciones): ?>
				<tr>
					<td class="w3-display-container day">
						<span class="w3-display-middle dayname">
							<?= (new \Cake\I18n\Date($dateKey))->i18nFormat($this->request->is('mobile') ? 'EEEEE' : 'EEEE') ?>
						</span>
					</td>
					<td class="grid">
						<table class="w3-table w3-table-all by-time">
							...
							<?php foreach ($asignaciones as $asignacion): ?>
								<tr>
									...
									<td><i class="fa-solid fa-user w3-hide-small"></i>
										<?= $asignacion->locutor->name ?>
									</td>
									<td>
										<?= $asignacion->horario->horaInicio ?> <i class="fa-solid fa-arrow-right-long"></i>
