...
<div style="padding:0.01em 16px;color:#fff!important;background-color:#2A4B7C!important;">
	<h3><?= $rol->fechaInicio->i18nFormat("d 'de' MMMM") ?> al <?= $rol->fechaFin->i18nFormat("d 'de' MMMM 'de' YYYY") ?>
	</h3>
</div>
		...
		<?php foreach ($asignaciones as $dia => $asignaciones): ?>
			<tr style="background:#<?= ($outterTR++ % 2 == 0) ? 'eee' : 'fff'; ?>">
				...
				<td style="padding:0 !important;">
					<table
						style="border-collapse:collapse;border-spacing:0;width:100%;display:table;margin:0;border:1px solid #ccc;">
						...
						<?php foreach ($asignaciones as $asignacion): ?>
							<tr style="background:#<?= ($innerTR++ % 2 == 0) ? 'fff' : 'eee'; ?>">
								...
								<td style="padding:8px;"><?= $asignacion->horario->horaInicio ?> &#10230;
									<?= $asignacion->horario->horaFin ?></td>
							</tr>
						<?php endforeach; ?>
					</table>
				</td>
